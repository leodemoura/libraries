...
:=
  have H3 : xx a + yy c + yy b = yy a + xx c + yy b, from
    calc
     xx a + yy c + yy b = xx a + yy b + yy c : add_right_comm _ _ _
      ... = yy a + xx b + yy c : {H1}
...
theorem quotient : is_quotient rel prod_sub rep
...
--absolute value
definition abs : ℤ → ℕ := rec_constant quotient (fun v, asub (xx v) (yy v))

theorem abs_comp (v : ℕ ## ℕ) : abs (prod_sub v) = asub (xx v) (yy v)
:=
  comp_constant quotient
    (take v w : ℕ ## ℕ,
      assume H : rel v w,
      show asub (xx v) (yy v) = asub (xx w) (yy w), from asub_eq H)
    (rel_refl v)


theorem abs_nzpos (n:nat) : abs (nzpos n) = n
:=
  calc
    abs (nzpos n) = asub (xx (tpair n 0)) (yy (tpair n 0)) : abs_comp _
      ... = asub n 0 : by simp
      ... = n : asub_zero_right n

theorem abs_nzneg (n:nat) : abs (nzneg n) = n
:=
  calc
    abs (nzneg n) = asub (xx (tpair 0 n)) (yy (tpair 0 n)) : abs_comp _
      ... = asub 0 n : by simp
      ... = n : asub_zero_left n
...
:=
  calc
    n = abs (nzpos n) : symm (abs_nzpos n)
  ... = abs (nzpos m) : {H}
  ... = m : abs_nzpos m
...
:=
  calc
    n = abs (nzneg n) : symm (abs_nzneg n)
  ... = abs (nzneg m) : {H}
  ... = m : abs_nzneg m
...
:=
  calc
    a * b = prod_sub (rep a) * prod_sub (rep b) : by simp
      ... = prod_sub (tpair (xx (rep a) * xx (rep b) + yy (rep a) * yy (rep b))
                            (xx (rep a) * yy (rep b) + yy (rep a) * xx (rep b)))
              : {mul_comp (rep a) (rep b)}
      ... = prod_sub (tpair (xx (rep b) * xx (rep a) + yy (rep b) * yy (rep a))
                            (xx (rep b) * yy (rep a) + yy (rep b) * xx (rep a)))
              : by simp
      ... = prod_sub (rep b) * prod_sub (rep a) : {symm (mul_comp (rep b) (rep a))}
      ... = b * a : by simp

theorem mul_zero_right (a : ℤ) : a * 0 = 0
:=
  calc
    a * prod_sub (tpair 0 0) = prod_sub (rep a) * prod_sub (tpair 0 0) : by simp
      ... = prod_sub (tpair (xx (rep a) * xx (tpair 0 0) + yy (rep a) * yy (tpair 0 0))
                            (xx (rep a) * yy (tpair 0 0) + yy (rep a) * xx (tpair 0 0)))
              : {mul_comp (rep a) (tpair 0 0)}
      ... = prod_sub (tpair 0 0) : by simp

-- theorem mul_zero_left (n:ℤ) : 0 * n = 0
-- := _

-- theorem mul_distr_right (n m k : ℤ) : (n + m) * k = n * k + m * k
-- :=
--   _

-- theorem mul_distr_left (n m k : ℤ) : n * (m + k) = n * m + n * k
-- := _
...
theorem foo (a b c : ℤ) : a + b + c = a + (b + c)
:=
  calc
    a + b + c = prod_sub (rep a) + prod_sub (rep b) + prod_sub (rep c) : by simp
      ... = prod_sub (map_pair2 nat::add (rep a) (rep b)) + prod_sub (rep c)
...
-- theorem mul_left_comm (n m k : ℤ) : n * (m * k) = m * (n * k)
-- := left_comm mul_comm mul_assoc n m k

-- theorem mul_right_comm (n m k : ℤ) : n * m * k = n * k * m
