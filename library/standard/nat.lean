...
:=
  or_elim (le_total n m)
    (assume H3 : n ≤ m, subst (H2 (m - n) (symm (add_sub_right H3)))
      (symm (le_imp_sub_eq_zero H3)))
    (assume H3 : m ≤ n, subst (H1 (n - m) (symm (add_sub_right H3)))
      (symm (le_imp_sub_eq_zero H3)))
...
:=
  calc
    asub n m = 0 + (m - n) : {le_imp_sub_eq_zero H}
      ... = m - n : add_zero_left (m - n)

theorem asub_ge {n m : ℕ} (H : n ≥ m) : asub n m = n - m
:= subst (asub_le H) (asub_comm m n)

theorem asub_zero_right (n : ℕ) : asub n 0 = n
:= trans (asub_ge (zero_le n)) (sub_zero_right n)

theorem asub_zero_left (n : ℕ) : asub 0 n = n
:= trans (asub_le (zero_le n)) (sub_zero_right n)

theorem asub_intro {n m k : ℕ} (H : n + m = k) : asub k n = m
:=
  calc
    asub k n = k - n : asub_ge (le_intro H)
      ... = m : sub_intro H

theorem asub_add_right (n m k : ℕ) : asub (n + k) (m + k) = asub n m
:=
  calc
    asub (n + k) (m + k) = n - m + ((m + k) - (n + k)) : {sub_add_add_right n m k}
      ... = n - m + (m - n) : {sub_add_add_right m n k}

theorem asub_ge_add_right {n m : ℕ} (H : n ≥ m) : asub n m + m = n
:=
  calc
    asub n m + m = n - m + m : {asub_ge H}
      ... = n : add_sub_left H

theorem asub_add_left (n m k : ℕ) : asub (k + n) (k + m) = asub n m
:= subst (subst (asub_add_right n m k) (add_comm n k)) (add_comm m k)

theorem asub_eq {n m k l : ℕ} (H : n + m = k + l) : asub n k = asub l m
:=
  have special : ∀n m k l, n + m = k + l → m ≤ l → asub n k = asub l m,
    from
      take n m k l,
      assume H : n + m = k + l,
      assume H2 : m ≤ l,
      have H3 : k + asub l m + m = n + m, from
        calc
          k + asub l m + m = k + (asub l m + m) : add_assoc _ _ _
            ... = k + l : {asub_ge_add_right H2}
            ... = n + m : symm H,
      have H4 : k + asub l m = n, from add_cancel_right H3,
      asub_intro H4,
